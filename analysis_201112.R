...
# GI stuff
dt$gi <- as.numeric(dt$diarrhoea == "t" | dt$vomiting == "t")
dt$newgi <- dt$gi
dt[same==0, newgi := 0]
r <- ftable(dt[country == "uk"]$week, dt[country=="uk"]$newgi,
            row.vars=1)
...
gi.12 <- gi.raw.data[-c(1:3, 22:26),]
write.csv(gi.12, "gi_201112.csv", quote=F, row.names=F)
