...
clean_and_merge <- function(data)
{
    ...
    for (name in names(files))
    {
        ...
        dt <- copy(data)
        dt <- dt[!duplicated(dt[, list(global_id, date)], fromLast=T)]

        ## Monday of the week
        for (col in grep("\\.date$", colnames(dt), value = TRUE))
        {
            week.col <- sub("\\.date$", ".week.date", col)
            dt[, paste(week.col) := get(col) - wday(get(col)) + 2]
        }
        ...
        } else if (name == "background")
        {
            ...
            ## calculate auxiliary variables: living with children, using public transport
